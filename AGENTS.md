...
- 所有回應與文件使用繁體中文
- ⚡ **圖檔下載約定**：當使用者詢問「哪裡下載」或索取圖檔路徑時，**必須直接執行 `open` 命令自動彈出 macOS Finder 視窗與圖片預覽**（例如 `open <assets_dir>` 與 `open <image_file>`），無須讓使用者手動複製路徑。
...
- [ ] 執行官網 v3.0 前端程式碼升級 (`index.html`, `styles.css`, `app.js`)
